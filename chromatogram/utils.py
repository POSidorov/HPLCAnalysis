...
def plot(ch:Chromatogram, wl_index:Optional[int]=None, peaks:bool=False, gaussians:bool=False, cut:bool=False):
    ...
    fig.update_layout(
        xaxis = dict(
            ...
            ticktext = [i*ch.info["time_interval"] for i in np.arange(np.ceil(ch.intensity[wl_index].index[0]/500)*500,
                                   np.ceil(ch.intensity[wl_index].index[-1]/500)*500,500)]
        )
    )
    fig.update_layout(title_text=ch.filename+"<br>Wavelength: "+str(int(ch.info["start_wl"]+wl_index*ch.info["wl_interval"]))+"nm", title_x=0.5)
